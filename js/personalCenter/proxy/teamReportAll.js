...
function teamReportAllInit(){
    ...
    $("#teamReportAllUlId").empty();
    var $ReportAllLUL=$('<ul class="recordDetail"><li>直  属 ： <span id="child_all"></span></li>' +
        '<li>团  队 ： <span id="team_all"></span></li>' +
        '<li>充值总额：<span id="charge_all"></span></li>' +
        '<li>提款总额：<span id="withdrawal_all"></span></li>' +
        '<li>购彩总额：<span id="buyLottery_all"></span></li>' +
        '<li>返点总额：<span id="point_all"></span></li>' +
        '<li>中奖总额：<span id="prize_all"></span></li>' +
        '<li style="display: none;">日工资：<span id="dailywage_all"></span></li>' +
        '<li>手 续 费：<span id="feeMoney_all"></span></li>' +
        '<li>其他收入：<span id="others_all"></span></li>' +
        '<li>盈利总额：<span id="winTotal_all"></span></li></ul>');
...
function teamReportAllSuccessCallBack(data) {
	if (data.SystemState == 64) {
		...
		$("#feeMoney_all").html(data.ReportComm[0].FeeMoneyTotal);
