...
function getRegistUrlListCallBack(data) {
    ...
    if (data.SystemState == 64 ){
        ...
        for(var i=RegistUrlList.length-1; i>=0; i--) {
            ...
            if (val.Status == 1){
                list += '注册地址：<span style="font-size: 13px;" class="redtext"> (长按复制)</span><textarea readonly cols="40" rows="2" style="width:100%;color:#b89a48;font-size:17px;padding:10px 0 0 5px; border: 1px solid #cccccc;margin-bottom:10px;">'+ getShortLink(val.RegistUrl) +'</textarea>';
