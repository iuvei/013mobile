...
function registerSubordinateInit(){
	webUrl = localStorageUtils.getParam("WEBUrl");
   ajaxUtil.ajaxByAsyncPost1(null, '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetUserDetail"}', getUserDetail_proxy);
  ...
  $("#registerSubmit").on("click", function() {
    ...
    // 注册生成链接按钮事件 - Old
    $("#btnHyperlink_old").off('click');
    $("#btnHyperlink_old").on("click", function() {
        var fandian_ = $("#HfanDianID").val();
        ajaxUtil.ajaxByAsyncPost(null, '{"ProjectPublic_PlatformCode":2,"InterfaceName":"SetUserPromotionRebate","Rebate":' + fandian_ + '}', SetUserPromotionRebate_old, '正在提交数据中...');
    });
}

//@ 注册生成链接按钮事件 - 013 Old
function SetUserPromotionRebate_old(data) {
    if (data.SystemState == 64) {
        if (data.IsComplete) {
            document.getElementById('hyperlinkid').style.display = "";
            $("#hyperlinkid").val(webUrl + myUserID);
        }
        ...
        if (data.RegisterComplete) {
            ...
            setTimeout(function () {
                $.ui.popup(
                    {
                        title:"注册成功",
                        message:'<p style="text-align: center;color:#e12048;margin:0;">长按以下内容可复制</p><textarea readonly="readonly" style="line-height: 25px;text-align:center;height:100px;resize:none;">'+ msg +'</textarea>',
                        cancelText:"关闭",
                        cancelCallback:
                            function(){
                            },
