...
function proxyAccountRecordInit(){
    ...
    //0:全部; 18:充值; 26:提款; 3:转账; ....
    var $selectType_proxyAccount = $('<select name="proxyAccountSearchType" id="proxyAccountSearchType" data-theme="a" data-mini="true" onchange="typeChangeProxy()"><option selected="selected" value="18">充值</option><option value="26">提款</option><option value="3">转账</option><option value="1">投注</option><option value="2">中奖</option><option value="4">撤单</option><option value="5">撤奖</option><option value="6">活动</option><option value="7">下级返点</option><option value="8">自身投注返点</option><option value="9">给下级充值</option><option value="10">来自上级的充值</option><option value="11">管理员添加</option></select>');

    if (localStorageUtils.getParam("IsDayWages") == "true"){
        $selectType_proxyAccount.append('<option value="29">日工资</option>');
    }else{
        $selectType_proxyAccount.append('<option value="24">日工资</option>');
    }
