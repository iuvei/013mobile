...
function teamBettingRecordsInit(){
    ...
    var $select=$('<table><tr>' +
        '<td><select name="searchDate_teamBetting" id="searchDate_teamBetting" data-theme="a" data-mini="true" onchange="dateChange_teamBetting()"><option value="0" selected="selected">当前记录</option><option value="1">历史记录</option></select></td>' +
        '<td><input type="text" id="selectDateTB_Stt" readonly/></td>' +
        '<td><input type="text" id="selectDateTB_End" readonly/></td></tr>' +
        '<tr><td colspan="3"><select name="searchType_teamBetting" id="searchType_teamBetting" data-theme="a" data-mini="true" onchange="lotteryChange_teamBetting()"></select></td></tr></table>');

    $("#teamBettingSelect").append($select);
    //查询开始时间
    selDateTBStart = new MobileSelectDate();
    selDateTBStart.init({trigger:'#selectDateTB_Stt',min:initDefaultDate(-3,"day"),max:initDefaultDate(0,"day")});
    selDateTBEnd = new MobileSelectDate();
    selDateTBEnd.init({trigger:'#selectDateTB_End',min:initDefaultDate(-3,"day"),max:initDefaultDate(0,"day")});
...
function createTeamBettingRecordsList(data){
    // console.log("调用查询TeamBetting历史投注记录方法返回的数据" + JSON.stringify(data));
    if (page == 0) {
        $("#teamBettingList").empty();
        ...
        for (var i = 0; i < Info.length; i++) {
            if(1==Info[i].SaleState && Info[i].LotteryCode!=99 && LotteryId.hasOwnProperty(Info[i].LotteryCode)){
                /*if(Info[i].LotteryCode==12){
                    $("#selectLottery_team").append('<option id='+i+' value=' + Info[i].LotteryCode + ' selected="selected">' + LotteryInfo.getLotteryNameById(Info[i].LotteryCode) + '</option>');
                }else{*/
...
function nextPage_teamBetting(startDateTime, endDateTime, lotteryType) {
    var params = '{"InterfaceName":"GetTeamBetDataListNew","SourceCode":1,"ProjectPublic_PlatformCode":2,"LotteryCode":' + lotteryType + ',"IsHistory":' + IsHistory + ',"insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamBetting + '}';
...
/**
 * 日期改变事件
 * [dateChange_teamBetting description]
...
function queryTeamBettingUserName(searchUser){
    page = 0;
    ajaxUtil.ajaxByAsyncPost(null, '{"LikeUserName":"'+searchUser+'","InterfaceName":"GetTeamBetDataListNew","LotteryCode":' + lotteryType + ',"SourceCode":1,"ProjectPublic_PlatformCode":2,"IsHistory":' + IsHistory + ',"insertTimeMin":"' + startDateTime + '","insertTimeMax":"' + endDateTime + '","CurrentPageIndex":' + page + ',"CurrentPageSize":' + PAGESIZE_teamCharge + '}',
