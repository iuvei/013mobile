...
function dateChangeGamePersonalReport() {
    ...
    switch(type) {
        ...
        case "1":
            ...
            endDateTime = $("#selectDateGamePersonalReport_End").val()+hms59;
            searchTotal_gamePersonalReport(startDateTime, endDateTime);
            changeDateRange_GamePersonalReport(-89,"day",-1,"day");     //Controller
            break;
    }
}
...
function gamePersonalReportChangeShow(data) {
    var selectedShow=0;
    selectedShow = $("#selectType_gamePersonalReport").val();

    if(data.ReportComm && selectedShow==1) {
        //Total
        $("#gamePersonalReportContent").empty();
        var $totalInfo = $('<ul class="recordDetail"><li>合计：<span id="gameDate"></span></li><li>总消费：<span id="gameConsume"></span></li><li>总获取：<span id="gameGet"></span></li><li>总房费：<span id="gameHouseFee"></span></li><li>总盈亏：<span id="totalPL"></span></li></ul>');
        $("#gamePersonalReportContent").append($totalInfo);

        $("#gameDate").html(data.ReportComm[0].HisDate);
        $("#gameConsume").html(data.ReportComm[0].GamePay);
        $("#gameGet").html(data.ReportComm[0].GameGet);
        $("#gameHouseFee").html(data.ReportComm[0].RoomFee);
        $("#totalPL").html(data.ReportComm[0].PL);
    }
    ...
    if(data.Reportlst && selectedShow==0) {
        ...
        for(var j=0;j<data.Reportlst.length;j++){
            ...
            $liContent.append('<ul class="recordDetail"><li>日期：<span>'+reportList[j].HisDate
                +'</span></li><li>游戏消费：<span>'+reportList[j].GamePay
                +'</span></li><li>游戏获取：<span >'+reportList[j].GameGet
                +'</span></li><li>房费：<span>'+reportList[j].RoomFee
                +'</span></li><li>自身盈亏 ：<span>'+reportList[j].PL
                +'</span></li></ul>');
