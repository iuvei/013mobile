...
function teamGameReportSubordinateInit(){
    ...
    var $select=$('<table><tr>' +
        '<td><select name="searchDate_teamGameReportSub" id="searchDate_teamGameReportSub" data-theme="a" data-mini="true" onchange="dateChange_teamGameReportSubordinate()"><option value="0" selected="selected">当天记录</option><option value="1">历史记录</option></select></td>' +
    ...
    if (null != conditions) {
        ...
        switch (dateChange){
            case "0":
                ...
                break;
            case "1":
                changeDateRange_GRSub(-89,"day",-1,"day");     //Controller
                break;
        }
...
function teamGameReportSubordinate_searchSuccessCallBack(data){
    ...
    if (data.SystemState == 64  && data.DataCount !=0) {
        ...
        for (var i = 0; i < info.length; i++) {
            ...
            $itemLi.append('<a class="recordList"><dl class="orderList"><dd>用户名:&nbsp;' + dataSet.userName + '</dd><dd>游戏消费:&nbsp;<span class="red">' + dataSet.GamePay +'</span></dd><dd>类型:&nbsp;'+dataSet.category+'</dd><dd>游戏获取:&nbsp;' + dataSet.GameGet +'</dd></dl></a>');
