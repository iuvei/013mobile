...
function modifyDailyWagesThree_callBack(data) {
    ...
    if (data.SystemState == 64){
        ...
        if(MyDayWages.length > 0){
            ...
            $("#my_daily_wages").html("我的日工资契约："+bigNumberUtil.multiply(MyDayWages[0].DayWagesProportion,100)+"%");
            ...
            if (initDailyWages.length > 0){
                for(var i = 0; i < initDailyWages.length; i++){
                    if(parseFloat(my_daily_wages) >= initDailyWages[i].DayWagesProportion){
                        var $LiMine = $('<li><span>'+bigNumberUtil.multiply(initDailyWages[i].DayWagesProportion,100) +'% </span><span>'+initDailyWages[i].DaySales+'</span><span>'+initDailyWages[i].ActiveNumber+'</span></li>');
        ...
        if(SubordinateDayWages.length > 0){
            ...
            $("#current_daily_wages").html("当前日工资："+bigNumberUtil.multiply(SubordinateDayWages[0].DayWagesProportion,100)+"%");
        }else{
        ...
        if (modifyDayWageInfo.State == 2) {  //添加日工资契约
            ...
            for (var i = 0; i < initDailyWages.length;i++){
                if(bigNumberUtil.multiply(initDailyWages[i].DayWagesProportion,100) >= data.MaxdDayWagesProportion && parseFloat(initDailyWages[i].DayWagesProportion) <= parseFloat(my_daily_wages)){
                    $("#new_daily_wages").append('<option value="'+initDailyWages[i].ID+'">每天【'+bigNumberUtil.multiply(initDailyWages[i].DayWagesProportion,100)+'%】的日工资</option>')
                }
            }
        }else{//修改日工资契约
            for (var i = 0; i < initDailyWages.length;i++){
                if(parseFloat(initDailyWages[i].DayWagesProportion) > parseFloat(current_daily_wages) && parseFloat(initDailyWages[i].DayWagesProportion) <= parseFloat(my_daily_wages)){
                    $("#new_daily_wages").append('<option value="'+(i+1)+'">每天【'+bigNumberUtil.multiply(initDailyWages[i].DayWagesProportion,100)+'%】的日工资</option>')
                }
            }
            if(modifyDayWageInfo.State == 0){
                $("#modifyDailyWagesBtn").hide();
                $("#new_daily_wages").hide();
                $("#pending_daily_wages").html('待确认日工资：'+bigNumberUtil.multiply(data.SuperiorUserDayWages,100)+'%');
            }
        }
...
function submitModifyDailyWages(modifyDayWageInfo) {
	...
	if (modifyDayWageInfo.State == 2){  //添加日工资契约
		ajaxUtil.ajaxByAsyncPost1(null,'{"InterfaceName":"AddDayWagesThree","ProjectPublic_PlatformCode":2,"UserName":"' + modifyDayWageInfo.userName +'","ID":'+ $("#new_daily_wages").find("option:selected").val() +'}', function (data) {
			if(data.SystemState == 64){
				if(data.Result == 1){
