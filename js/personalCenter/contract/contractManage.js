...
function contractManageLoadedPanel(){
    ...
    if (localStorageUtils.getParam("IsDayWages") == "true"){
        $("#showMyDailywages").hide();   //013 暂不显示日工资3
...
function newDaywageMsg(data) {
    ...
    if (data.State == 0){
        ...
        $("#showMyDailywages").find("a.contractManageBtn").css('background',"#e12048").html("我的契约");
    }
}
...
function newFenHongMsg(data) {
    ...
    if (data.State == 0){
        ...
        $("#showMyContract").find("a.contractManageBtn").css('background',"#e12048").html("我的契约");
    }
}