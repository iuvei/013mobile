...
function modifyBankPasswordInit(){
	...
	$("#modifyBankWpdmitId").on('click', function() {		
		...
		if (newWithdrawalPasswd == confirmWithdrawalPasswd){
			if(newWithdrawalPasswd == "a123456" || newWithdrawalPasswd == "123456a" || newWithdrawalPasswd == "123456"){
		...
		if ("1" == isHasPayPwd) {   //Type:0 -- 登录和资金密码可以一致，Type:1 -- 登录和资金密码不可一致。
		    var param = '{"ProjectPublic_PlatformCode":2,"Type":0,"LogPassword":"' + oldWithdrawalPasswd + '","NewPassword":"' + confirmWithdrawalPasswd + '","InterfaceName":"ModifyPayPass"}';
		    ajaxUtil.ajaxByAsyncPost1(null, param, changeWithdrawalPwdCallBack_zj,null);
		} else {
		    var param_zj = '{"ProjectPublic_PlatformCode":2,"Type":0,"NewPassword":"' + confirmWithdrawalPasswd + '","InterfaceName":"ModifyPayPass"}';
