...
function lotteryHallLoadedPanel() {
    var params = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetUserAllMoney"}';
       ajaxUtil.ajaxByAsyncPost1(null, params, function(data){
            if(data.SystemState == "-2" || data.SystemState == "-1") {
                loginAgain();
            }else{
                if(localStorageUtils.getParam("isLogin") == 'true') {
                    ...
                    //充值银行
                    var param = '{"ProjectPublic_PlatformCode":2,"InterfaceName":"GetUserGroupPaymentNew"}';
                    ajaxUtil.ajaxByAsyncPost1(null, param, getUserPayment_callBack, null);
    ...
    $("#awardHallId").on('click', function(){
     ...
     //充值
     $("#index_rechargeId").off('click');
     $("#index_rechargeId").on('click', function() {
         if(localStorageUtils.getParam("isLogin") == "true"){
             //获取充值银行信息
     ...
     //提款
     $("#index_withdrawId").off('click');
     $("#index_withdrawId").on('click', function() {
         if(localStorageUtils.getParam("isLogin") == "true"){
             flag = 2;
             //接口调用，判断用户有几张卡已绑定
			CheckBankCardNumber();
    ...
    //登录后   提示绑定手机号；
	 if( !localStorageUtils.getParam("userPhone") && localStorageUtils.getParam("myLottery_enterType") == "0"){
		 // 遮罩层
		 var $shadowBg = $('<div id="shadowBg" ' +
			 'style="width: 100%;height: 100%; background-color: #111;opacity: 0.6;position: fixed;z-index: 99999;">' +
			 '</div>');
		 $("body").append($shadowBg);

		 $.ui.popup(
			 {
				 title:"提示",
				 message:'手机号用作您的身份认证，用于找回密码，</br>请您认真填写您的手机号',
				 cancelText:"关闭",
				 cancelCallback:
					 function(){
						 localStorageUtils.setParam("myLottery_enterType","1");
						 //去掉遮罩
						 if($("#shadowBg")){
							 $("#shadowBg").remove();
						 }
