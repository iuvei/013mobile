...
function myReportChangeShow(data) {
	...
	selectedShow = $("#myReportSearchType").val();
	  ...
	  if(data.ReportComm && selectedShow==1) {
		//Total
		$("#myReportContent").empty();
		var $totalInfo = $('<ul class="recordDetail"><li>充值总金额：<span id="totalCharge"></span></li><li>提款总金额：<span id="totalWithdrawal"></span></li><li>购彩总金额：<span id="totalCost"></span></li><li>总奖金：<span id="myWinAwards"></span></li><li>自身返点：<span id="myRebate_self"></span></li><li>下级返点：<span id="subordinateRebate_sub"></span></li><li>其 他 ：<span id="getFromOthers"></span></li><li>盈利总金额：<span id="totalPrize"></span></li></ul>');
	  ...
	  if(data.Reportlst && selectedShow==0) {
		...
		for(var j=0;j<data.Reportlst.length;j++){
			...
			$liContent.append('<ul class="recordDetail"><li>时间：<span>'+reportList[j].HisTime.split(" ")[0]
				...
				+'</span></li><li>其 他 ：<span>'+ bigNumberUtil.add(reportList[j].OtherMoney,reportList[j].Dailywage)
