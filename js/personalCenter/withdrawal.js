...
function withdrawalLoadedinit(){
	...
	ajaxUtil.ajaxByAsyncPost1(null, param, function (data) {
        if(data.SystemState == 64){
            realUserNameWithdraw = data.UserRealName;
            userBankWithdraw = data.BankCode;
            userBankNoWithdraw = data.CardNumber;
            drawMinMoney = data.DrawMinMoney;
            drawMaxMoney = data.DrawMaxMoney;
            drawBeginTime = data.DrawBeginTime;
            drawEndTime = data.DrawEndTime;
	...
	//获取当前用户的银行卡列表信息
    var params_bankList='{"ProjectPublic_PlatformCode":2,"UserID":'+ myUserID +',"InterfaceName":"GetBankCardList"}';
	ajaxUtil.ajaxByAsyncPost1(null, params_bankList,function (bankData) {
		if(bankData.SystemState == -1){
			loginAgain();
		}else {
            var BankCardList = bankData.BankCardList;
            $.each(BankCardList,function (key,val) {
                if(val.Isdefault){
                    defaultBankPro = val.Province;       //默认银行卡省份
                    defaultBankCity = val.City;          //默认银行卡城市
                    defaultBankBranch = val.BranchName;  //默认银行卡支行
                }
            })
		}
    },null);
...
function postwithdrawalSubmit(){
	...
	if(!defaultBankBranch || !defaultBankCity || !defaultBankPro || defaultBankCity == "0" || defaultBankCity == "-1" || defaultBankPro == "0" || defaultBankPro == "-1"){
		toastUtils.showToast("您的提款卡开户行信息未完善，不能提款，请联系客服！");
		return;
	}
