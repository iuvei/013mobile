		...
		if(getLayoutParam("shortcut")){
			...
			$buttonOdd.on('click',function(){
				...
				if(getLayoutParam("maxBallsNum") == 8) { //北京快乐8 任性二至任选七 最多选8个号
					...
					if(lotteryType == "esf" || lotteryType == "pks" || lotteryType == "k3" || lotteryType == "kl8"){
...
function queryLastPrize(lottery){
	...
	if(lotteryId == 50){
		...
		ajaxUtil.ajaxByAsyncPost(null,'{"InterfaceName":"GetLotteryDrawPageFlx","CurrentPageSize":5,"LotteryCode":"50","BeginTime":"' + startDateTime + '","ProjectPublic_PlatformCode":2,"EndTime":"' + endDateTime + '","CurrentPageIndex":0}',function(data){
			$("#"+lottery+"_prizelist div").first().empty();
			if (data == null || data == "" || data.length == 0 || typeof(data) == "undefined" || data.IssueModlst == null) {
				$("#"+lottery+"_prizelist div").first().append("暂无最新开奖");
			}else{
				var $table = $('<table></table>');
				$table.append('<tr ><th style="width:20%">期号</th><th style="width:80%">开奖结果<img src="./././images/reload.png" style="width: 24px;height:24px;margin:0 20px 0 -46px;float:right;" id="'+lottery+'reloadRst"></th></tr>');
				$.each(data.IssueModlst, function(key, val) {
					var _resultS = val.DrawResult1;
					_resultS = _resultS.replace(new RegExp(/,/g)," ");
					$table.append('<tr ><td>'+val.IssueNumber1+'</td><td style="color:red;">'+_resultS+'</td></tr>');
				});
				$("#"+lottery+"_prizelist div").first().append($table);
				//点击刷新图标重新载入最新开奖结果
		...
		ajaxUtil.ajaxByAsyncPost1(null, '{"IsSelf":false,"ProjectPublic_PlatformCode":2,"Size":5,"Page":0,"reType":1,"InterfaceName":"GetHisNumber","CZID":"' +  lotteryId + '"}',function(data){
			$("#"+lottery+"_prizelist div").first().empty();
			if (data === null || data === "" || data.length === 0 || typeof(data) ==="undefined") {
				...
				$.each(data, function(key, val) {
					if(key < 5) {
						...
						_resultS = _resultS.replace(new RegExp(/,/g), " ");
						$table.append('<tr ><td>' + val.CzPeriod + '</td><td style="color:red;">' + _resultS + '</td></tr>');
					}
				});
				$("#"+lottery+"_prizelist div").first().append($table);
				//点击刷新图标重新载入最新开奖结果
...
function initFooterData(lotteryName,playMethod) {
	...
	var Default_=localStorageUtils.getParam(lotteryId+"");
	var startRebate = 0.0;
	if (isCloseFandian) {
		fandianshu = 1000;
...
function  setDefault(lotteryName,playMethod,startRebate){
	...
	var tempFandian = parseFloat(startRebate); //如实显示返点，从最小开始遍历
	// var tempFandian = parseFloat(bigNumberUtil.divided((parseInt(defaultBetRebate) - parseInt(minRebate)),20));  // 虚拟显示返点

	for (var k = defaultBetRebate; k >= minRebate; k -=xRebate, tempFandian += 0.1) {
		/*tempFandian= Number(tempFandian.toFixed(1));
		 var fandiantemp=k + "," + tempFandian;*/

		var temp = "" + tempFandian.toFixed(2);
		temp = temp.substring(0, temp.length - 1);
		var fandiantemp=k + "," + temp;
...
function createSingleLayout(lotteryName,tips){
	...
	var temp = "<textarea style=\"overflow-x:hidden; height:130px \" id=\""+lotteryName+"_single\" onkeyup='"+onkeyup+"' onblur=\""+lotteryName+"ValidateData(this)\" />";
...
function RepeatArr(arr){
	...
	for (var i = 0; i < arr.length; i++) {
		if ((arrStr.match(new RegExp(arr[i],"g")).length)>1){
			repetitiveArray.push(arr[i]);
		}
	}
	return uniqueArr(repetitiveArray);
}
