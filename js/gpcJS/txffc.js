...
function txffc_init(){
	...
	for(var i = 0; i< LotteryInfo.getPlayLength("ssc");i++){
        if (i == 0 || i == 1 || i == 10 || i == 11 || i==12 || i==13 || i==14 || i==15){//去掉五星，四星，和任选2.3.4玩法,趣味，龙虎，骰宝龙虎
            continue;
        }
		var $play = $('<optgroup label="'+LotteryInfo.getPlayName("ssc",i)+'"></optgroup>');
		for(var j = 0; j < LotteryInfo.getMethodLength("ssc");j++){
			//去掉不定位四星/五星玩法,三星前三直选单式、中三直选单式、后三直选单式、前三混合组选、中三混合组选、后三混合组选，二星前二组选复式，后二组选复式。
			if( (i==8 && j >= 67 && j <= 73) || (i==2 && j==16) || (i==2 && j==23) || (i==3 && j==27) || (i==3 && j ==34) || (i==4 && j==38) || (i==4 && j==45) || (i==5 && j==52) || (i==6 && j==59) ){
				continue;
			}
 ...
 * @return {[type]}      [description]
 */
function txffcValidateData(data){
	txffcValidData(data.value);
}

function txffcValidData(str){
	var notes = 0;
	var array;
	if(txffc_playMethod == 1){
		...
		notes = array.length * mathUtil.getCCombination($("#txffc_tab .button.red").size() ,2);
		txffcShowFooter(true,notes);
		...
		notes = array.length * mathUtil.getCCombination($("#txffc_tab .button.red").size() ,3);
		txffcShowFooter(true,notes);
		...
		notes = array.length * mathUtil.getCCombination($("#txffc_tab .button.red").size() ,4);
		txffcShowFooter(true,notes);
	}
