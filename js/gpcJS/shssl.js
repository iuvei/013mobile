...
function shsslChangeItem(val) {
	...
	if(val == "shssl0"){
		...
		$("#shssl_ballView").empty();
		shssl_qingkongAll();
		var tips = "<p>格式说明<br/>前二组选:12<br/>1)每注必须是2个号码,每个号码之间无需用符号分割,（2个号码必须各不相同）;2)每注之间用空格分割;3)只支持单式.</p>";
		createSingleLayout("shssl",tips);
	}else if(val == "shssl10"){
		//直选复式
		$("#shssl_random").show();
		...
		$("#shssl_ballView").empty();
		shssl_qingkongAll();
		var tips = "<p>格式说明<br/>后二组选:12<br/>1)每注必须是2个号码,每个号码之间无需用符号分割,（2个号码必须各不相同）;2)每注之间用空格分割;3)只支持单式.</p>";
		createSingleLayout("shssl",tips);
	}else if(val == "shssl15"){
		//直选单式
		$("#shssl_random").show();
...
function shsslValidateData(data){
	shsslValidData(data.value);
}

/**
 * [shsslValidData 单式数据验证]
 */
function shsslValidData(content){
	var array;
