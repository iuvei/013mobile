...
function plwChangeItem(val) {
	...
	if(val == 'plw0'){
		...
		plw_playMethod = 1;
		plw_qingkongAll();
		var tips = "<p>格式说明<br/>直选:12345<br/>1)每注必须是5个号码,每个号码之间无需用符号分割;2)每注之间用空格分割;3)只支持单式.</p>";
...
function plwValidateData(data) {
	plwValidData(data.value);
}
 ...
 * @return {[type]}      [description]
 */
function plwValidData(content){
	var array = handleSingleStrNew({
		str:content,
		weishu:5
	});
